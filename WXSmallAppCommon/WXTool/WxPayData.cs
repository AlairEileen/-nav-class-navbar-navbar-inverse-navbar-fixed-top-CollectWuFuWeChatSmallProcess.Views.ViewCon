...
namespace WXSmallAppCommon.WXTool
{
    public class WxPayData
    {
        ...
        * @throws WxPayException
        */
        public SortedDictionary<string, object> FromXml(string xml,Func<string,string> keyFunc)
        {
            ...
            try
            {
                ...
                object attach;
                m_values.TryGetValue("attach",out attach);
                var keyParam = "";
                if (attach!=null)
                {
                    string[] aa = attach.ToString().Split(',');
                    keyParam = aa[0];
                }



                CheckSign(m_values, keyFunc(keyParam));//验证签名,不通过会抛异常
